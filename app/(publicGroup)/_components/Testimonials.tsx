            ...
            <BlueprintCard key={item.id} className="p-5 flex flex-col justify-between" accentTick>
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-mono-spec text-[9px] bg-[#1F4D3E]/10 text-[#1F4D3E] px-2 py-0.5 rounded border border-[#1F4D3E]/20 font-bold">
                    {item.badge}
                  </span>
                </div>
                <p className="text-xs sm:text-sm text-[#1B211E]/90 leading-relaxed italic">
                  “{item.quote}”
                </p>
              </div>

              <div className="flex items-center gap-3 pt-4 mt-4 border-t border-[#D8DBD3]">
                {item.avatar ? (
                  <img
                    src={item.avatar}
                    alt={item.name}
                    className="w-9 h-9 rounded-full border border-[#D8DBD3] object-cover"
                  />
                ) : (
                  <div className="w-9 h-9 rounded-full bg-[#1F4D3E] text-white flex items-center justify-center font-heading font-bold text-xs">
                    {item.name.charAt(0)}
                  </div>
                )}
                <div>
                  <div className="font-heading font-bold text-xs text-[#1B211E]">
                    {item.name}
                  </div>
                  <div className="font-mono-spec text-[10px] text-[#1F4D3E] font-medium">
                    {item.role}
                  </div>
                </div>
              </div>
